...
//g' >>${TMP_PATH}/diskinfo.log
    ...
    cat ${var} |awk -F";" '/^9/ {print $2}'|awk '{print $1"|"$2}' | sed "s/^/${CHECKDATE}|"${var%_*}"|/"|sed 's/^M//g' >>${TMP_PATH}/diskinfo.log
  fi
fi
...
for logname in `echo "host_ip ethnet hbafc scsiinfo tapeinfo cdrominfo kernelinfo swapinfo lvm_vg lvm_lv lvm_pv softversion oraclever oracleuser oracletbs oracleparameter tuxedo peizhiinfo filesysteminfo diskboot sysenv vcs-haclus-ip vcs-haclus-sys vcs-haclus vcs-hasys vcs-hagrp vcs-hares vcs-hagrp-froze vcs-lltstat"`
do
