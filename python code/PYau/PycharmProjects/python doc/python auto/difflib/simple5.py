...
def compareme(dir1,dir2):#递归获取更新项函数
    ...
    diff_in_one=dircomp.diff_files #不匹配文件源目录文件已经发生变化
    ...
    if len(dircomp.common_dirs) > 0:   #判读是否存在相同子目录,以便递归
...
def main():
    ...
    print source_files
    if not dir2.endswith('/'): dir2=dir2+'/'   #备份目录路径+/
    ...
    for item in source_files:
        ...
        if os.path.isdir(item):   #如果差异入境为目录且不存在,则在备份目录中创建
            if not os.path.exists(destination_dir):
                createdir_bool=True     #再次调用 compareme 函数标记

    if createdir_bool:      #重新调用 compareme 函数,重新遍历新创建的目录的内容
        destination_files=[]
        source_files=[]
    ...
    copy_pair=zip(source_files,destination_files)  #将源目录和备份目录文件清单拆分成元组
