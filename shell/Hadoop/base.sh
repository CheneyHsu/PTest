...
if [ $? = 0 ];then
    ...
    #bonding setup
    echo "alias bond0 bonding" > /etc/modprobe.d/bond0
...
#添加账户和SSH-key（root,hadoop,hbase）
groupadd bgdtgrp
useradd -g bgdtgrp hadoop
useradd -g bgdtgrp hbase
echo hbase | passwd --stdin hbase
echo hadoop | passwd --stdin hadoop
for i in {hadoop,hbase};do
echo "ssh-keygen -t rsa -P '' -f /home/$i/.ssh/id_rsa ; exit" | su - $i
done
}

function ntp(){
#NTP server
echo "server 35.1.7.19" >> /etc/ntp.conf
echo "server 35.1.7.20" >> /etc/ntp.conf
#DNS
#echo "nameserver 130.1.9.54" > /etc/resolv.conf
}
