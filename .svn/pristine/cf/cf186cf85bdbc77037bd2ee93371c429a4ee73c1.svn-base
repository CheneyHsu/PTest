...
A_list = [
'mkdir /tmp/log',
'extract -f /tmp/log/global.txt -g -xp > /dev/null 2>&1',
'extract -f /tmp/log/application.txt -a -xp > /dev/null 2>&1',
'extract -f /tmp/log/transaction.txt -t -xp > /dev/null 2>&1',
'extract -f /tmp/log/process.txt -p -xp > /dev/null 2>&1',
'extract -f /tmp/log/disk.txt -d -xp > /dev/null 2>&1',
'extract -f /tmp/log/volume.txt -z -xp > /dev/null 2>&1',
'extract -f /tmp/log/network.txt -n -xp > /dev/null 2>&1',
'extract -f /tmp/log/filesystem.txt -y -xp > /dev/null 2>&1',
'extract -f /tmp/log/cpu.txt -u -xp > /dev/null 2>&1',
'extract -f /tmp/log/globala.txt -G -xp > /dev/null 2>&1',
'extract -f /tmp/log/applicationa.txt -A -xp > /dev/null 2>&1',
'extract -f /tmp/log/transactiona.txt -T -xp > /dev/null 2>&1',
'extract -f /tmp/log/diska.txt -D -xp > /dev/null 2>&1',
'extract -f /tmp/log/volumea.txt -Z -xp > /dev/null 2>&1',
'extract -f /tmp/log/networka.txt -N -xp > /dev/null 2>&1',
'extract -f /tmp/log/filesystema.txt -Y -xp > /dev/null 2>&1',
'extract -f /tmp/log/cpua.txt -U -xp > /dev/null 2>&1',
'extract -f /tmp/log/localsystem.txt -I -xp > /dev/null 2>&1',
...
#    os.system("mkdir -p /tmp/log/iowrite`cat /tmp/log/iowrite`;for i in {cmdline,cpuset,environ,io,limits,maps,mountinfo,mounts,mountstats,numa_maps,sched,smaps,status};do cat /proc/`cat /tmp/log/iowrite`/$i > /tmp/log/iowrite`cat /tmp/log/iowrite`/$i > /dev/null 2>&1;done")
    os.system("A=`cat /tmp/log/ioread`;mkdir -p /tmp/log/ioread$A;for i in {cmdline,cpuset,environ,io,limits,maps,mountinfo,mounts,mountstats,numa_maps,sched,smaps,status};do cat /proc/$A/$i > /tmp/log/ioread$A/$i;done")
    os.system("A=`cat /tmp/log/iowrite`;mkdir -p /tmp/log/iowrite$A;for i in {cmdline,cpuset,environ,io,limits,maps,mountinfo,mounts,mountstats,numa_maps,sched,smaps,status};do cat /proc/$A/$i > /tmp/log/iowrite$A/$i;done")
...
#    os.system("mkdir -p /tmp/log/topcpu`sed -n 1p /tmp/log/topcpu|awk \'{print $2}\'`;for i in {cmdline,cpuset,environ,io,limits,maps,mountinfo,mounts,mountstats,numa_maps,sched,smaps,status};do cat /proc/`sed -n 1p /tmp/log/topcpu|awk \'{print $2}\'`/$i > /tmp/log/topcpu`sed -n 1p /tmp/log/topcpu|awk \'{print $2}\'`/$i > /dev/null 2>&1 ;done")
    os.system("A=`sed -n 1p /tmp/log/topcpu|awk \'{print $2}\'`;mkdir -p /tmp/log/topcpu$A;for i in {cmdline,cpuset,environ,io,limits,maps,mountinfo,mounts,mountstats,numa_maps,sched,smaps,status};do cat /proc/$A/$i > /tmp/log/topcpu$A/$i ;done")
...
#    os.system("mkdir -p /tmp/log/topmem`sed -n 1p /tmp/log/topmem|awk \'{print $2}\'`;for i in {cmdline,cpuset,environ,io,limits,maps,mountinfo,mounts,mountstats,numa_maps,sched,smaps,status};do cat /proc/`sed -n 1p /tmp/log/topmem|awk \'{print $2}\'`/$i > /tmp/log/topmem`sed -n 1p /tmp/log/topmem|awk \'{print $2}\'`/$i > /dev/null 2>&1 ;done")
    os.system("A=`sed -n 1p /tmp/log/topmem|awk \'{print $2}\'`;mkdir -p /tmp/log/topmem$A;for i in {cmdline,cpuset,environ,io,limits,maps,mountinfo,mounts,mountstats,numa_maps,sched,smaps,status};do cat /proc/$A/$i > /tmp/log/topmem$A/$i;done")
