...
declare
sleep_time       constant  NUMBER(3)  :=  3;
display_time     constant  CHAR(3)    := 'N';
display_ratio    constant  CHAR(1)    := 'Y';
...
db_plat_p     varchar2(20);   -- database planform
...
select substr(replace(banner,'TNS for '),1,instr(replace(banner,'TNS for '),':')-1) into db_plat_p from v$version where banner like 'TNS for %';
